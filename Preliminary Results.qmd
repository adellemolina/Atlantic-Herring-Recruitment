...
The combined dataset here came from various sources (mostly NOAA ecodata github) and were modified, summarized, compiled and exported in a separate script.

-   SST, BT, SSS & BS raw data came from the NMFS survey and were clipped to EPU regions (based on Sean Lucey survdat package)
    -   Basin wide annual values were computed as area weighted means
    -   Regional annual means were also included
    -   Seasonal basin wide values were computed as area weighted means
-   Environmental indices came from ecodata
    1.  Heatwave index (Basin wide area weighted mean and regional means)
    2.  Cold Pool index
    3.  Gulf Stream Index
-   Food
    -   Abundance of calanus finmarchicus stages (C3-adult) with season and region variables
    -   Zooplankton abundance by region and taxa (cnidaria, euphausids, small calanoid, large calanoid)
    -   Density of small zooplankton by region, season and taxa (centropages, pseudocalanus & temora)
-   Predators
...
-   Biological indices
    -   Chlorophyll a (basin wide area weighted)
    -   Primary production (basin wide and regional)
-   Herring recruitment indices came from 2022 ASAP model
...
![](Figures/Recruitment%20Indices.tiff)
