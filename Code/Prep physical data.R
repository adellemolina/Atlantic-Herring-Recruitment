...
##      Notes:            Confused about data sources but I think the SST I have downloaded manually is the right one
...
# shit this is saved onto my desktop...load in and combine and save to HD as a csv
OISST <- read.csv()
...
#3.  CTD (not done playing with this...compare to RAW, but only goes to 1996...?)
# survey temp and sal at depth  (CTD)
NEFSC.ctd  <- read.csv(file.choose(), header = T, blank.lines.skip = T) #  temperature and salinity at depth
range(NEFSC.ctd$year)
# Add ecoregion (get spatial polygons for Ecological Production Units (EPUs) 
EPU <- epu_sf

#set projection
sf::st_crs(x = 4269)

# Create sf data, project, and store as data frame
CTD <- as.data.frame(NEFSC.ctd) %>% 
  st_as_sf(coords= c("longitude", "latitude"), crs=4269)

#subset for just GOM
EPU.GOM <- EPU[EPU$EPU == "GOM",]

# add new column
NEFSC.CTD.GOM <- st_join(CTD, left = F, EPU.GOM["EPU"]) # join points
...
# Calculate annual means for GoM
copepod <- copepod%>%
  mutate_if(is.character, as.factor)%>%
